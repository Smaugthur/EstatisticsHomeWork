...
namespace STATISTICS
{
    ...
    class StatisticsSimple : Statistics
    {
        public float[] frecuencyValues;
        public StatisticsSimple(float[] inputData)
        {
    ...
    class StatisticsWithIntervals : Statistics
    {
        public float[,] frecuencyValues;
        public StatisticsWithIntervals(float[] inputData, int intervalsLength)
        {
            this.inputData = inputData;
            frecuencyValues = GetFrecuencyValues(this.inputData, intervalsLength);
            absoluteFrecuency = GetAbsoluteFrecuency(this.inputData, frecuencyValues);
            cumulativeFrecuency = GetCumulativeFrecuency(absoluteFrecuency);
            relativeFrecuency = GetRelativeFrecuency(absoluteFrecuency);
        }
        public float[,] GetFrecuencyValues(float[] inputData, int intervalsLength)
        {
            ...
            for(int i = 0; i < intervals.GetLength(0); i++)
            {
                ...
                intervals[i,1] = intervals[i,0] + intervalsLength - 1;
            }
            return intervals;
        }
